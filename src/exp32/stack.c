 ...
 * PJJ110 (OPPO Reno11 Pro, 5.10.226): UNMEASURED — S22U geometry retained
 * as the 5.10 same-generation default until a live measurement exists.
 * The two kernels share futex_wait_requeue_pi frame shape (SP write-set
 * audit: prologue sub sp,#0x1a0, Δ=+0x90, W=SP+0x90) but vendor edits in
 * task_blocks_on_rt_mutex (PJJ110 BUG_ON triples at 0x1ea860) change the
 * compat copy path; if the chain never lines up, re-measure W via the
 * perf SP leak (docs/5.10-pjj110.md) and set EXP32_STAMP_OFF = W - gr32. */
